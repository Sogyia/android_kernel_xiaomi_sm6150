...
static void proc_command_line_init(void) {
    ...
    char *offset_addr = NULL;
    char *hwc_offset_addr = NULL;
    char hwc_value[8] = "";
...
#ifdef CONFIG_CMDLINE_HWC_IS_SKU
    if (hwc_offset_addr) {
        char *space_ptr = strstr(hwc_offset_addr + HWC_STR_LEN, " ");
        if (space_ptr) {
            memcpy(hwc_value, hwc_offset_addr + HWC_STR_LEN, space_ptr - (hwc_offset_addr + HWC_STR_LEN));
